...
if __name__ == "__main__":
    ...
    QSExp = M.DoQuickStretches(PSetExp, ifPlot=True, ifSave=False, ifTD=True)
    exp_mean = np.array([QSExp[j][0] for j in range(len(QSExp))])
    exp_std = np.array([.003]*len(QSExp)) 
    ...
    for ip in range(1, len(PoI_all)):
        ...
        Features_QStd = M.DoQuickStretches(PSet, ifPlot=False, ifSave=False, ifTD=True)
        # with open('Features_QStd.dat', 'rb') as file_features:
        #     [PSet, Features_QStd] = pickle.load(file_features)
        
        
        Emulators = [None]*len(FoI)
        for iFeat, Feat1 in enumerate(FoI):
            Emulators[iFeat] = G.CreateEmulator(Feat1, X[:, jPoI], Features_QStd)
    ...
    #         emulator.append(emul)
        ...
        W.plot_wave(xlabels=xlabels, display="impl", filename=f'{home}/Python/HMtest_{ip}')
