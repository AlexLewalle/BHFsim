...
print('Doing FpCa')
Features = {**Features, **M.DoFpCa(PSet, Lambda0=1.0)}       # update Features dictionary with FpCa entries

dLambda = 0.1
print(f'Doing FpCa with a different SL={1+dLambda}')
Features_1 = M.DoFpCa(PSet, Lambda0=1+dLambda)
...
figS1, axS1 = plt.subplots(numrows, numcols, figsize=([14,  9]))
figSummary, axSummary = plt.subplots()
...
for iFeat, Feat1 in enumerate(WhichFeatures):
    ...
    dataset = Dataset(X_train=X, 
                      y_train= Features[Feat1], #Fmin_a, #Fpeak_a, #EC50_a, #nH_a, #Fmax_a,
